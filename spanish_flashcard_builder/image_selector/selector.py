...
class ImageSelector:
    ...
    def _load_augmented_term(self, term_dir: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logging.error(f"Unexpected error loading augmented term from {term_dir}: {e}")
            return None
    ...
    def _search_images_for_term(self, term_dir: Path, term_data: Dict[str, Any]) -> Optional[List[str]]:
    ...
    def _handle_selection_result(self, term_dir: Path, selection_index: Optional[int], 
                               selected_image: Optional[Image.Image]) -> bool:
    ...
    def _process_single_term(self, term_dir: Path) -> bool:
        ...
        selection_index, selected_image = self._handle_image_selection(image_urls, term_data)
        
        return self._handle_selection_result(term_dir, selection_index, selected_image)

    def _handle_image_selection(self, image_urls: List[str], term_data: Dict[str, Any]) -> Tuple[Optional[int], Optional[Image.Image]]:
