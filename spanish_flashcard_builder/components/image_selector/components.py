...
from .constants import (
    TITLE_FONT, SUBTITLE_FONT, BODY_FONT, ITALIC_FONT,
    PREVIEW_SIZE, GRID_COLUMNS, PADDING
)
...
class TermInfoPanel:
    ...
    def _add_example_sentences(self):
        ...
        for i, example in enumerate(self.term_data['example_sentences'], 1):
            ttk.Label(
                self.frame,
                text=f"{i}. {example.get('es', '')}",
                font=BODY_FONT
            ).pack(anchor='w')
            
            ttk.Label(
                self.frame,
                text=f"   {example.get('en', '')}",
                font=ITALIC_FONT
    ...
    def display_images(self, loaded_images: List[tuple[int, Image.Image]]) -> None:
        for grid_idx, (image_idx, image) in enumerate(loaded_images):
            ...
            for widget in (frame, number_label, img_label):
                widget.bind(
                    '<Button-1>',
                    lambda e, idx=image_idx: self.on_select(idx)
                )
