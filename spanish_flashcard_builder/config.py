...
class _Paths:
    ...
    def __init__(self):
        ...
        self.cleaned_vocab = self.data_dir / config.get_value(PATHS, DATA, 'cleaned_vocab')
        self.selector_history = self.data_dir / config.get_value(PATHS, DATA, 'selector_history')
        ...
        self.dictionary_entry_filename = config.get_value(PATHS, OUTPUT, TERMS, COMPONENTS, 'dictionary_entry')
        self.augmented_term_filename = config.get_value(PATHS, OUTPUT, TERMS, COMPONENTS, 'augmented_term')
...
class _OpenAI:
    ...
    def validate(self) -> None:
        ...
        if not 0 <= self.temperature <= 1:
            raise ConfigError(f"OpenAI temperature must be between 0 and 1, got {self.temperature}")
        if not self.model:
